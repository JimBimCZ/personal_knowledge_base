...
the source document. Built as a home assignment for the *AI Implementation Expert* role
(option B). The grading is about **how the work was done**, not how it looks.

Explicitly **not** graded, so do not spend budget there: visual design, feature count, test
coverage, lines of code.

Explicitly graded, so spend budget here:

1. It runs on the first attempt by following the README (`docker compose up`, nothing else).
2. The author can explain any line of code and any design decision at the interview.
...
**Time budget: 8 hours total.** When a task threatens the budget, cut scope and write the cut into
`README.md` → *Known gaps / deliberate debt*. Never silently skip a mandatory condition in §3.

## 2. The prime directive

> Never write code the author would not be able to defend out loud.
...
- One concept, one file, obvious name. If a reviewer has to jump through three files to see how a
  question becomes an answer, the design is wrong.
- Every non-trivial decision gets one line in `docs/decisions.md`:
  `YYYY-MM-DD — <decision> — <why> — <what was rejected>`. This file is the interview cheat sheet.
...
## 3. Hard requirements (from the assignment — non-negotiable)
...
- No direct call to a public AI API anywhere outside `src/server/ai/providers/`. The rest of the
  codebase only knows the interfaces in §5. This is the single most-inspected part of the
  assignment: a corporate AI Gateway must be a new provider file plus an env var.
- API keys come from env vars only.
- Speech-to-text: not needed for option B. Say so in one line in the README.
...
- **Anonymization is mandatory and must be visible.** Before any text leaves the process toward a
  ...
  the placeholders are restored. Both directions must be demonstrable in the UI/demo video.
...
- README contains the **Master Prompt** (the brief used to generate the app) plus model name and
  version. Keep it updated as the brief evolves; it is a deliverable, not a souvenir.
...
after the first is exactly the kind of thing this assignment is inspecting.
...
- **The app must start and be fully demoable with no API key set** so a reviewer without one still
  gets a working system. That is the acceptance test for both seams.
...
"Delete my account" wipes all rows belonging to the `sub` and is demoed in the video.

## 8. Working agreements for Claude

- Before coding a slice, restate in one sentence which mandatory condition from §3 it satisfies.
- Build in vertical slices that keep the app runnable at every commit:
  ...
  6. README, Master Prompt, `prompts/`, decisions log, demo pass (~1h)
...
- When you hit the time budget on a slice, stop, and write the remainder into README → *Known gaps*.
  An honest gap scores; a half-finished feature does not.
...
- [ ] README has: run instructions, model + version, Master Prompt, Entra ID swap section, MFA
      rationale, retention table, classification + reasoning, known gaps.
- [ ] `docs/decisions.md` covers every decision the author would be asked "why like this?" about.
- [ ] 3–5 minute walkthrough recorded.

## 10. Classification (draft — the author confirms the final wording)

Proposed: **MEDIUM**. Reasoning to be argued in the README: it processes personal data contained in
user documents and sends derived text to an external model, which raises the data-protection
profile above a trivial tool; but it serves a single user's own material, holds no company-critical
process, has no integrations into production systems, and an outage is an inconvenience rather than
an incident. The anonymization layer and the retention policy lower residual risk, they do not
remove it. If the reviewer's classification matrix defines the levels differently, follow the
matrix and keep the reasoning structure.
