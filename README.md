...
Built as a home assignment for the *AI Implementation Expert* role, topic **B**. Everything
in `seed/` is synthetic — the people, e-mail addresses and phone numbers in it are invented.
...
```bash
git clone https://github.com/JimBimCZ/personal_knowledge_base.git
...
**Speech-to-text is not needed for topic B**, so none is implemented. Were it required, it
would be a third interface beside the two above, selected by its own environment variable,
...
The brief that governs this build lives in **[`CLAUDE.md`](./CLAUDE.md)** in the repository
root. It is not a souvenir of the first prompt — it is the living specification, corrected as
the build found things the original brief got wrong (most notably that Anthropic has no
embeddings endpoint, which split one interface into two). `docs/implementation-plan.md`
records what the brief left open, and `docs/decisions.md` records every decision taken since.

**Generated with:** Claude Opus 5 (`claude-opus-5`), via Claude Code.
...
>
> **Prime directive: never write code the author would not be able to defend out loud.** Prefer
...
>
> Time budget: 8 hours. When a task threatens the budget, cut scope and write the cut into the
> README under *Known gaps*. An honest gap scores; a half-finished feature does not.

Prompts sent to the model are **not** in this README and not in the TypeScript. They are files
in [`prompts/`](./prompts), loaded from disk at runtime, so a reviewer can read exactly what
the model was told without reading any code, and editing a prompt is not a code change.
...
- **Nothing fake is baked into the image either.** `next build` imports every route module,
  ...
  of a repository graded on secret hygiene. The build phase is detected and skipped instead,
...
The assignment does not supply a classification matrix, so the criteria used here are stated
explicitly. **If your matrix defines the levels differently, follow your matrix and keep the
reasoning structure.**
...
incident rather than a visible bug. Test coverage is explicitly not graded, and chasing it
would have spent budget that the decisions log needed.
...
9. **`users.role_snapshot` can go stale.** It is refreshed at sign-in and used only for
   ...
   but a reviewer should know it is there and why it is not authoritative.
