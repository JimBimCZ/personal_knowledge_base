...
This is a stronger answer to the assignment, not a weaker one: the mandatory condition is
that LLM calls sit behind a swappable abstraction, and there are demonstrably two different
external services here. Collapsing them into one interface would have hidden that.
...
- **The model is baked into the image at build time.** `env.localModelPath` plus
  ...
  runtime download would have violated the zero-manual-steps requirement the first time a
  reviewer ran it on a bad connection.
...
`docker compose up` with no API key has to give a reviewer a working demo. Random vectors
would score below `RAG_MIN_SCORE` on every query, the citation guard would correctly reject
every answer, and the reviewer would see "Not found in your knowledge base." for everything —
a broken-looking app that is technically behaving correctly.
