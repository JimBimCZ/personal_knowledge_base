...
## Slice 6 — README, Master Prompt and the honest record

2026-08-26 — The README's demo steps were verified against a fresh `git clone` before being written down, not after — the first graded criterion is "does it run on the first attempt following the README", and a README written from memory of a machine that already has `.env`, `node_modules` and a warm model cache is exactly how that criterion is failed; the clean run confirmed three healthy containers, migrations applied, 10 documents and 53 chunks seeded on first sign-in, with no manual step — rejected writing the instructions from the developer's own working directory.

2026-08-26 — Anonymization is demonstrated by TWO questions, one per direction, rather than one claiming to show both — the first draft told the reviewer to ask about "Marek Dvorak" and promised the answer would come back with names restored; run against the fresh clone, the mock picked sentences containing no personal data at all, so the inbound half of the round trip was invisible. Outbound is now shown by a question containing a name, inbound by one whose answer contains a name, e-mail and phone. Found by running the README's own instructions — rejected the single-question version, which would have failed in front of the reviewer.

2026-08-26 — The classification section states the criteria it is judging against, because the assignment supplies no matrix — a verdict of "MEDIUM" with no stated axes is an opinion; the same verdict against nine named criteria is an argument someone can disagree with specifically. It also says which two design choices are load-bearing (in-process embeddings, enforced retention) and that removing either moves the classification up — rejected asserting the level and leaving the reasoning implicit.

2026-08-26 — The Master Prompt section names `CLAUDE.md` as the living brief and reproduces its operative content condensed, rather than pasting the file — two verbatim copies of a 200-line specification drift, and the assignment asks for a brief that is kept current, not a souvenir; the section also records what the brief got wrong and where the correction lives, which is the part worth explaining at interview — rejected duplicating the file into the README.

2026-08-26 — Known gaps are ten specific, individually checkable statements, not a paragraph of hedging — "the anthropic provider has never been run against the live API" is a gap a reviewer can act on; "some parts are less tested" is not. Each says what is missing, why it was acceptable to stop there, and what it would take to close — the assignment grades what was deliberately left out while being aware of it, and awareness has to be demonstrable.

2026-08-26 — `docs/implementation-plan.md` §9 was closed rather than deleted, and its §7 gained a "deviation, as built" note — the plan is a record of what was believed before the build, and silently editing it to match the outcome destroys the only evidence of what changed and why; a reviewer reading a stale open item that has in fact been resolved is worse than either — rejected both deleting the section and leaving it stale.
...
2026-08-26 — A fourth `LlmProvider`, `openrouter`, was added specifically to run the abstraction against a real third-party gateway — the assignment's most-inspected requirement is that a corporate AI Gateway is a swap, and "here is a `gateway.ts` I never called" is a claim, not evidence; OpenRouter is a different company, account, billing relationship and model namespace, and reaching it cost one 20-line provider file with no change to the prompt, request, parsing, citation guard, anonymizer or audit record — rejected leaving `gateway.ts` as the only argument.
